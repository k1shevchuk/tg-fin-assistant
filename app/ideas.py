...
def _build_security_idea(ticker: str, board: str, tag: str) -> Optional[Idea]:
    ...
    thesis = _compose_thesis(ticker, metrics)
...
def _build_crypto_idea(coin_id: str, symbol: str) -> Optional[Idea]:
    ...
    thesis = (
        f"BTC/USD {price:.0f} USD, 7д изм. {change_7d:.1f}%" if change_7d is not None else f"BTC/USD {price:.0f} USD"
    )
...
def _compose_thesis(ticker: str, metrics: dict[str, float | str | None]) -> str:
    price = metrics.get("price") or 0.0
    dma50 = metrics.get("dma50")
    key_rate = get_key_rate() * 100
    parts = [f"{ticker.upper()} {price:.2f} {metrics.get('currency', 'RUB')}"]
    if dma50:
        parts.append(f">50DMA {dma50:.2f}")
    change = metrics.get("change_percent")
    if change is not None:
        parts.append(f"день {change:.2f}%")
    parts.append(f"ключевая {key_rate:.2f}%")
    return ", ".join(parts)
